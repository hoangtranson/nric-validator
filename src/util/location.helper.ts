export namespace LOCATION {
    ...
    const isMalaysia = (code: number) => numisBetween(code, 1, 16) || numisBetween(code, 21, 59) || code == 82;
    const isForeign = (code: number) => {
        const _country = Object.keys(COUNTRIES).find( _code =>  _code == `${code}`);
        return _country ? true : false;
    }
    const numisBetween = (num: number, lower: number, upper: number) => (num - lower) * (num - upper) <= 0;

    export const isStateValid = (code: number) => isMalaysia(code) || isForeign(code);
