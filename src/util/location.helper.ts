export namespace LOCATION {
  ...
  const numisBetween = (num: number, lower: number, upper: number) =>
    (num - lower) * (num - upper) <= 0;

  export const isStateValid = (code: number) =>
    isMalaysia(code) || isForeign(code);
}
